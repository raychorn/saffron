...
/**
 * Parses a UTF-8 string into a Cron value. Returns null if:
 *
 * * `s` is null,
 *
 ...
 */
const Cron *saffron_cron_parse(const char *s, size_t l);

/**
 * Frees a previously created cron value.
 */
void saffron_cron_free(const Cron *c);

/**
 * Returns a bool indicating if the cron value contains any matching times.
 */
bool saffron_cron_any(const Cron *c);
 ...
 */
bool saffron_cron_contains(const Cron *c, int64_t s);
 ...
 * The valid range for `s` is -8334632851200 <= `s` <= 8210298412799.
 */
bool saffron_cron_next_from(const Cron *c,
                            int64_t *s);
 ...
 * The valid range for `s` is -8334632851200 <= `s` <= 8210298412799.
 */
bool saffron_cron_next_after(const Cron *c, int64_t *s);
 ...
 * The valid range for `s` is -8334632851200 <= `s` <= 8210298412799.
 */
CronTimesIter *saffron_cron_iter_from(const Cron *c, int64_t s);
 ...
 * The valid range for `s` is -8334632851200 <= `s` <= 8210298412799.
 */
CronTimesIter *saffron_cron_iter_after(const Cron *c, int64_t s);
 ...
 */
bool saffron_cron_iter_next(CronTimesIter *c, int64_t *s);

/**
 * Frees a previously created cron times iterator value.
 */
void saffron_cron_iter_free(CronTimesIter *c);
